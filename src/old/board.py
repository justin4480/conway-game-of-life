...
class Board:
    def __init__(self, patterns, player):
        self.zeros = np.zeros(shape=(VideoSettings.HEIGHT, VideoSettings.WIDTH), dtype=np.uint8)
    ...
    def upsample(array: np.array):
        return array.repeat(VideoSettings.PIXEL_SIZE, axis=0).repeat(
            VideoSettings.PIXEL_SIZE, axis=1
        )
    ...
    def populate_patterns(self):
        for pattern in self.patterns:
            x_start, y_start = self.get_random_x_y_board_pos(pattern.array.shape)
            x_end, y_end = pattern.array.shape + np.array([x_start, y_start])
            self.board[x_start:x_end, y_start:y_end] = pattern.array

    def spawn_player(self, buffer=15):
        x_board, y_board = self.board.shape
        x_player, y_player = self.player.array.shape  # noqa:E231,F841
        ...
        self.board[x_buffer_start:x_buffer_end, y_buffer_start:y_buffer_end] = np.zeros(
            shape=(x_player + buffer * 2, y_player + buffer * 2)
        )
